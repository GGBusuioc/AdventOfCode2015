...
fs.readFileSync("./data.txt", "utf8").split('\n').forEach((line) => {
    ...
    const [l,w,h] = line.match(/\d+/g).map(Number);
    ...
    // find the smallers perimeter    
    let ribbonToWrapPresent = Math.min(p1, p2, p3)
    let ribbonForTheBow = l*w*h
    
    let totalPresentRibbon = ribbonToWrapPresent + ribbonForTheBow

    totalRibbon += totalPresentRibbon

    console.log(l + " " + w + " " + h + " " + totalPresentRibbon)
});
