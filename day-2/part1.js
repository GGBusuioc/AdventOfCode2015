...
fs.readFileSync("./data.txt", "utf8").split('\n').forEach((line) => {
    ...
    let giftPaper = 2*l*w + 2*w*h + 2*h*l
    let extraPaper = Math.min(l*w, w*h, h*l)
    
    let totalGiftPaper = giftPaper + extraPaper

    totalWrappingPaper += totalGiftPaper

    console.log(l + " " + w + " " + h + " " + totalGiftPaper)
});
